...
time in order to populate our database.
...
Let's name a fake moon landing "SpaceZ" and save the record:
...
relative dates indicating it's been about 50 years since the last landing:
...
The app can travel into the future, too! Suppose we [fast
forward](https://www.nasa.gov/press-release/nasa-provides-update-to-astronaut-moon-lander-plans-under-artemis)
a few years and record a landing by an Artemis I mission:

```
$ TRAVEL_TO=2027-07-15 ./script/server
```

Adding such a record and (yet again) restarting the server without the
`TRAVEL_TO` environment variable should result in a final lineup of moon
landings like this:
...
set as the default for the `mission_landings.landed_at` column. Because the
`landed_at` time is being set by the database as opposed to being passed as a
SQL query parameter, merely faking time in Ruby isn't good enough!
...
The `nowish()` function's implementation adds the value returned by
`pg_catalog.now()` to an offset of seconds that the application stores in a
table called
...
move the database (or at least the return value of `nowish()`) as well by
setting this global offset. And the [TravelsInTime](/app/lib/travels_in_time.rb)
...
that some amount of
[shenanigans](https://github.com/testdouble/time_traveler_demo/blob/main/config/initializers/perform_time_travel.rb#L12-L14)
are necessary to ensure that cookies will work when the current time is rewound
into the distant past relative to the user agent.)

That's just about all there is to it! With this combination of pieces in place,
a test can conceivably time-travel in even relatively complex app—so long
as each reference to the current time in one's database queries, views, and
...
[branch](https://github.com/testdouble/time_traveler_demo/tree/shadowing-builtin-now-function)
that shadows the `pg_catalog.now()` function in pre-production environments, but
will call through to the real one (see its commit messages for caveats and risks
that might entail).
